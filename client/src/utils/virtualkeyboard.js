let Vshift = false
    ...
    const showKeybordBtnhandler = (setLang, lang, socket, room) => {
      ...
      if (!shift) {
        for (let key in keysValues[i]) {
          if (keysValues[i][key].length === 2) {
            ...
            keys += `  <button value_data= "${key}" key_data = "${i}" id = "${keysValues[i][key]}" onclick="KeyClickhandler(this)" class="keys ${keysValues[i][key]}_key">${keysValues[i][key]}</button>`
          }
        }
      } else {
        for (let key in keyboard_ar_value[i]) {
          if (keysValues[i][key].length === 2) {
            ...
            keys += `  <button value_data= "${key}" key_data = "${i}" id = "${keysValues[i][key]}" onclick="KeyClickhandler(this)" class="keys ${keysValues[i][key]}_key">${keysValues[i][key].toUpperCase()}</button>`
          }
        }
      }
    ...
    const mainKeyboardhandler = (shift, setLang, lang, socket, room) => {
      let KeysHtm = ''
      if (lang === 'ar') {
      ...
      KeyControlerhandler(e).then((inp) => {
        if (!Valt) {
          ...
          if (Vshift) {
            if (lang === 'ar') {
              setLang('en')
              mainKeyboardhandler(Vshift, setLang, 'en', socket, room)
            } else {
              setLang('ar')
              mainKeyboardhandler(Vshift, setLang, 'ar', socket, room)
            }
          }
        } else {
          Valt = false
